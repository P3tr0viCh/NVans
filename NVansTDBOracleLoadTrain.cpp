...
void TDBOracleLoadTrain::Operation() {
	...
	try {
		...
		QueryText = SQLMake(QueryText, IDS_SQL_ORACLE_NVANS_SELECT);
		...
		QueryText = SQLMake(QueryText, IDS_SQL_ORACLE_NVANS_WHERE);
		QueryText = SQLMake(QueryText, IDS_SQL_ORDER);
		QueryText = SQLMake(QueryText, IDS_SQL_ORACLE_NVANS_ORDER);
